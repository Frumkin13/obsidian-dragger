...
import { DropTargetResolver, type DropTargetResolverDeps } from './drop-target-resolver';
...
function createDeps(overrides?: Partial<DropTargetResolverDeps>): DropTargetResolverDeps {
    return {
...
describe('DropTargetResolver', () => {
    it('computes a basic drop target from pointer position', () => {
        ...
        const calculator = new DropTargetResolver(view, createDeps());

        const validation = calculator.resolveValidatedDropTarget({ clientX: 40, clientY: 5 });

        expect(validation.allowed).toBe(true);
        expect(validation.resolution?.target.targetLineNumber).toBe(1);
        expect(validation.resolution?.preview.indicatorY).toBe(12);
    });
    ...
    it('allows non-list blocks to target the line above a list item', () => {
        ...
        const validation = calculator.resolveValidatedDropTarget({
            clientX: 40,
            clientY: 5,
            selection: createSource('outside', 5, 5),
        });
    ...
    it('rejects self-range drop before indicator rendering', () => {
        ...
        const validation = calculator.resolveValidatedDropTarget({
            clientX: 40,
            clientY: 5,
            selection: createListSource('- first', 0, 0),
        });
    ...
    it('returns no_anchor when insertion anchor cannot be resolved', () => {
        ...
        const validation = calculator.resolveValidatedDropTarget({
            clientX: 40,
            clientY: 5,
            selection: createOutOfDocumentSource('outside'),
            sourceScope: 'cross_editor',
        });
    ...
    it('targets after blank last line when pointer is in the lower half of that blank line', () => {
        ...
        expect(validation.allowed).toBe(true);
        expect(validation.resolution?.target.targetLineNumber).toBe(4);
    });
    ...
    it('prefers rendered cm-line hit-testing before posAtCoords for hr lines', () => {
        ...
        expect(validation.allowed).toBe(true);
        expect(validation.resolution?.target.targetLineNumber).toBe(3);
    });
});
