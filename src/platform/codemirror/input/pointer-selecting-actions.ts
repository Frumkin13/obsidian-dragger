...
export function beginRangeSelectionSessionAction(
    ...
    if (pointerType !== 'mouse') {
        dragTimeoutId = window.setTimeout(() => {
            ...
            const nextSource = host.resolveBlockSelection(host.buildDirectRangeSelectionSelectionRequest(state));
        ...
        dragTimeoutId = window.setTimeout(() => {
            ...
            state.dragReady = true;
            const nextSource = host.resolveBlockSelection(host.buildActiveRangeSelectionSelectionRequest(state));
            if (!nextSource) return;
            host.emitPressPendingLifecycle(nextSource, state.pointerType, true);
        }, MOUSE_RANGE_SELECT_LONG_PRESS_MS);
    }
    ...
    const initialSource = host.resolveBlockSelection(host.buildActiveRangeSelectionSelectionRequest(initialRangeSelectState));
