...
import { appendMarkdownBlock, FileMoveApplier } from './file-move-applier';

type MutableView = EditorView & {
    documentDispatchCount: number;
};

describe('FileMoveApplier', () => {
    it('appends a block to a closed target file and deletes it from the source editor', async () => {
        ...
        const result = await new FileMoveApplier(app).applyFileMove({
            sourceView,
            selection: selectionFromBlock(createBlock(1, 1)),
            targetFile,
        });
    ...
    it('moves composite selections into a target file as one appended block', async () => {
        ...
        const result = await new FileMoveApplier(app).applyFileMove({
            sourceView,
            selection: selectionFromBlock(createBlock(0, 2), [
    ...
    it('keeps a whole-file same-file move to one undoable dispatch without extra padding', async () => {
        ...
        const result = await new FileMoveApplier(app).applyFileMove({
            sourceView,
            selection: selectionFromBlock(createBlock(0, 0)),
            targetFile,
        });
